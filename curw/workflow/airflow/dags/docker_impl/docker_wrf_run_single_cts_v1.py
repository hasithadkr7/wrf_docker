...
wrf_dag_name = 'docker_wrf_run_single_cts_v1'
...
def generate_random_run_id(prefix, random_str_len=4, **context):
    run_id = '_'.join(
        [prefix, context['next_execution_date'].strftime('%Y-%m-%d_%H:%M'),
...
init_config = PythonOperator(
    ...
    templates_dict={
        'wrf_config_start_date': "{{ macros.datetime.strftime(next_execution_date , \'%%Y-%%m-%%d_%%H:%%M\') }}",
